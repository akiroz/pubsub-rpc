...
export type PubSubClient = {
    ...
    subscribe(topic: string, handler: (payload: Uint8Array) => void): Promise<void>;
    ...
    await client.subscribe(topic, async (msg) => {
        try {
            ...
            const response = await handler(params)
                .then((result) => ({ result }))
                .catch((error) => ({ error }));
            await client.publish(`${topic}/${strId}`, MsgPack.encode(response));
        } catch(err) {
...
export async function call(client: PubSubClient, topic: string, params: any = {}, opt = { timeout: 10000 }): Promise<any> {
    ...
    const msg = await Promise.race([
        new Promise((rsov, rjct) => ee.once("resp", (payload) => {
            client.unsubscribe(responseTopic);
            rsov(payload);
        })),
        new Promise((rsov, rjct) => setTimeout(() => {
            client.unsubscribe(responseTopic);
